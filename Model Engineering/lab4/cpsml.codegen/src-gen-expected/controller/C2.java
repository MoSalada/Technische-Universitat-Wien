...
public class C2 implements MessagingElement {
	...
	public C2(String name, DataSource<String> dataSource) {
		this.name = name;
		this.topics = Arrays.asList("temp");
	...
	public void addPublication(Linkable mB, List<String> topics) {
		...
		System.out.println("    🟦 C2 added new Publication on " + topics.toString() +" to "+ mB.toString());
	}

	@Override
	public void addSubscription(Linkable mB, List<String> topics) {
		subscriptionLinks.merge(mB, topics, (s1,s2)->mergeStringLists(s1, s2));
		System.out.println("    🟥 C2 added new Subscription on " + topics.toString() +" to "+ mB.toString());
	}
